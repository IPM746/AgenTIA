...
export const runAgentTask = async (
    ...
    const log = (msg: string) => callbacks.onLog ? callbacks.onLog(msg) : console.log(msg);
...
2. Cuando hayas terminado, escribe un breve resumen.`;
        ...
        while (iteracion <= maxIteraciones) {
            ...
            const currentPromptTokens = usage?.promptTokens 
                ?? Math.ceil(historyCharsCount / 4);
                
            const currentCompletionTokens = usage?.completionTokens 
                ?? Math.ceil((response.text?.length || 0) / 4);
                
            const currentTotalTokens = usage?.totalTokens 
                ?? (currentPromptTokens + currentCompletionTokens);
                
            const isEstimated = usage?.estimated ?? (usage === undefined);
            ...
            if (response.toolCalls && response.toolCalls.length > 0) {
                for (const call of response.toolCalls) {
                    ...
                    if (callbacks.onToolResult) callbacks.onToolResult(call.name, result);
                ...
                console.log(`\n📊 [Métricas - Baseline]`);
                console.log(`Origen datos: ${metrics.isEstimated ? 'Estimación local o mixta' : 'Tokens reales del proveedor'}`);
                console.log(`Iteraciones: ${metrics.iterations}`);
                console.log(`Tool calls: ${metrics.toolCalls}`);
                console.log(`Input tokens (acumulados): ${metrics.promptTokens}`);
                console.log(`Output tokens (acumulados): ${metrics.completionTokens}`);
                console.log(`Total tokens: ${metrics.totalTokens}\n`);

                console.log(`📈 [Crecimiento del contexto por iteración]`);
                metrics.iterationDetails.forEach(det => {
                console.log(
                  `Iteración ${det.iteration} → input: ${det.promptTokens} | output: ${det.completionTokens} | historial: ${det.historyMessages} msgs (~${det.historyChars} chars)`
                  );               
               });
                console.log("");

                if (callbacks.onFinish) callbacks.onFinish(responseText, metrics);
                break;
            }
            iteracion++;
        }
        ...
        if (iteracion > maxIteraciones) {
            ...
            if (callbacks.onFinish) callbacks.onFinish("Límite de iteraciones alcanzado.", metrics);
        }
